...
def tweet_text(tweet):
    ...
    "*** YOUR CODE HERE ***"
    return tweet[0]
#We just "extract" the text portion of the tweet.

def tweet_time(tweet):
    """Return the datetime representing when a tweet was posted."""
    "*** YOUR CODE HERE ***"
    return tweet[1]
#We just "extract" the time portion of the tweet.

def tweet_location(tweet):
    """Return a position representing a tweet's location."""
    "*** YOUR CODE HERE ***"
    return make_position(tweet[2], tweet[3])
#Looking back in the geo.py file, we noticed that make_position takes
#in lat and lon, so all we did was feed in the proper arguments of 
#lat and lon from the tweets in the make_position function. 
...
def extract_words(text):
    ...
    if string != "":
        pile.append(string)
    return pile
# Explanation: The else statement allows us to build the word. The combination
# of the if and elif statement allows us to efficiently account for spaces.
# The first if tests whether or not we're at the end of the word. If we are,
# we want to put the completed word into a "pile", which is the function of the
# elif statement. Finally, we want to return the "pile" that we've been
# adding the completed words to. 
...
def make_sentiment(value):
    ...
    "*** YOUR CODE HERE ***"
    return value
# Assert statement accounts for the falsey values, so we just return the value.

def has_sentiment(s):
    """Return whether sentiment s has a value."""
    "*** YOUR CODE HERE ***"
    if type(s) == type(0.2) or type(s) == type(0) or type(s) == type(1):
        return True
    return False
#Checks for if "s" has sentiment, which is denoted by the type 0.2 and 0.
...
def sentiment_value(s):
    ...
    return s
#Assert statement inspects for no sentiments. Otherwise, just return s
#to obtain the sentiment value. 
...
def analyze_tweet_sentiment(tweet):
    ...
    """
    "*** YOUR CODE HERE ***"
    list_of_sentiments = [get_word_sentiment(word) for word in extract_words(tweet_text(tweet))]

    sum_of_sentiments=[sentiment_value(x) for x in list_of_sentiments if type(x) == type(get_word_sentiment('out')) and has_sentiment(x) ]


  
    if sum_of_sentiments != []:
        sentiment_number = sum(sum_of_sentiments)
        return make_sentiment(sentiment_number/len(sum_of_sentiments))
    else:
        return make_sentiment(None)
#only passed 50% of tests.
...
def find_centroid(polygon):
    ...
    """
    i = 0
    lat_sum,long_sum,Area_sum = 0,0,0
    while i<(len(polygon)-1):
        some_formula = (latitude(polygon[i])*longitude(polygon[i+1]))-latitude(polygon[i+1])*longitude(polygon[i])
        lat_sum = lat_sum + (latitude(polygon[i])+latitude(polygon[i+1]))*some_formula
        long_sum = long_sum + (longitude(polygon[i])+longitude(polygon[i+1]))*some_formula
        Area_sum = Area_sum + some_formula
        i += 1
    Area_sum = Area_sum/2
    if Area_sum == 0:
        return [latitude(polygon[0]), longitude(polygon[0]),Area_sum ]
    lat_sum=lat_sum/(6*Area_sum)
    long_sum=long_sum/(6*Area_sum)
    return [lat_sum,long_sum,abs(Area_sum)]
...
def find_state_center(polygons):
    ...
    for polygon in polygons:
        ...
        centroid_latitude = find_centroid(polygon)[0]
        centroid_longitude = find_centroid(polygon)[1]
        centroid_area = find_centroid(polygon)[2]
        sum_x = sum_x + centroid_latitude*centroid_area
        sum_y = sum_y + centroid_longitude*centroid_area
        sum_area= sum_area+centroid_area

    x_state_center = sum_x/sum_area
    y_state_center = sum_y/sum_area

    return make_position(x_state_center,y_state_center)
...
def group_tweets_by_state(tweets):
    ...
    for x in tweets:
        ...
        min_distance = 9000000000000000000
        closest_state = "canada"
        for state in us_states:
            shape = us_states[state]
            distance = geo_distance(tweet_location(x),find_state_center(shape))
            if distance < min_distance:
                min_distance=distance
                closest_state = state
 
        if closest_state not in dictionary:
            dictionary[closest_state]=[x]
        else:
            dictionary[closest_state]=dictionary[closest_state]+[x]
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        ...
        for tweets in tweet_list:
            sentiment_of_words = analyze_tweet_sentiment(tweets)
            if has_sentiment(sentiment_of_words):
                sum_of_sentiments = sum_of_sentiments +  sentiment_value(sentiment_of_words)
                tweets_with_sentiments +=1

                
            else:
                sum_of_sentiments=sum_of_sentiments


        if tweets_with_sentiments != 0:
            avg_sentiment = sum_of_sentiments/tweets_with_sentiments
            dictionary[state]=avg_sentiment
        else:
            avg_sentiment = None
    return dictionary
