...
def has_sentiment(s):
    ...
    if type(s) ==type(0.2) or type(s) == type(0):
        return True
    else:
        return False 
...
def analyze_tweet_sentiment(tweet):
    ...
    False
    """
    list_of_sentiments = [make_sentiment(get_word_sentiment(word)) for word in extract_words(tweet_text(tweet))]
    sum_of_sentiments=[x for x in list_of_sentiments if type(x)==type(0.2) or type(x)==type(1) or type(x)==type(0)]
    if sum_of_sentiments != []:
        sentiment_number = sum(sum_of_sentiments)
        return make_sentiment(sentiment_number)
    else:
...
def group_tweets_by_state(tweets):
    ...
    for x in tweets:
        ...
        for state in us_states:
            ...
            distances[distance]=state
        a = min(distances.keys())
        closest_state = distances[a]
