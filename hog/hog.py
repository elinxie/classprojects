...
def base_final_greater_than(score, opponent_score, margin=6, num_rolls=5):
    """Modified copy of swap_strategy which adds leave_four_sided
    """
    if swap_strategy(score,opponent_score,margin,num_rolls)==num_rolls:
...
def final_strategy(score, opponent_score,dice=six_sided):
    ...
    First we check whether we are rolling 4 or 6 sided dice. If we have 4 sided dice, we have lower
    num_roll because we are more likely to get a 1. 

    Second, we check if our score is higher or lower than our opponents. If we have a high score, not only
    do we lower our num_roll relative to the num_roll where we are lower than our opponents, we have 
    different commands for when we have a higher score. Those commands include the command to prevent
    the free bacon rule or the pig out rule to have us swine swap with the opponent.We also 
    implement a function to make our opponent's next dice four sided by the free bacon rule so that the 
    opponent cannot catch up to us. 
     When our score is 
    lower, we have the commands to roll 0 or a high number repectively if the rules free bacon or pig out
    allows us to switch with the opponent's higher score. We do not include the function to make 
    our opponent's dice to be four sided because we believe it is more important for us to get more 
    points than to have the opponent get a different dice. 

    If free_bacon can get me to above 100, the goal, then I call free_bacon
    """
    if (score + opponent_score) % 7 == 0:
        if score > opponent_score:
            return base_final_greater_than(score,opponent_score,3,3)
        elif score == opponent_score:
            return 5
        else:
            return base_final_less_than(score,opponent_score,5,5)
    else:
        if score > opponent_score:
