...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while score0<goal and score1<goal:
        if who == 0:
            score0 = take_turn(strategy0(score0,score1),score1)+score0
            dice = select_dice(score0,score1)
            score0,score1 = swine_swap(score0,score1)
            who=other(who)
            
        else:
            score1 = take_turn(strategy1(score1,score0),score0)+score1
...
def leave_four_sided(score, opponent_score):
    if (free_bacon(opponent_score)+score+opponent_score)%7==0:
        return True
def trigger_four_sided(score,opponent_score):
...
def base_final_less_than(score, opponent_score, margin=6, num_rolls=5):
    """Modified copy of swap_strategy which adds leave_four_sided
    """
    if 2*(score+free_bacon(opponent_score))==opponent_score:
        return 0

    elif trigger_swine(score,opponent_score):
        return 10

    elif leave_four_sided(score,opponent_score):
        return 0
    elif trigger_four_sided(score,opponent_score):
        return 10
    elif (score + opponent_score) % 7 ==0:
        return 3
    else:
        return num_rolls
def base_final_greater_than(score, opponent_score, margin=3, num_rolls=5):
    """Modified copy of swap_strategy which adds leave_four_sided
    """
    if score+free_bacon(opponent_score) == opponent_score*2:
        if (score + opponent_score) % 7 ==0:
            ...
            return 5    
    elif prevent_trigger_swine(score,opponent_score):
        return 1
...
def final_strategy(score, opponent_score):
    ...
    if score>opponent_score:
        return base_final_greater_than(score,opponent_score,4,4)
    else:
        return base_final_less_than(score,opponent_score,9,6)
