...
def apply_primitive(procedure, args, env):
    ...
    try: 
        ...
        raise SchemeError("too many/too few arguments for primitive function")
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        to_be_bound = target.first
        if scheme_symbolp(to_be_bound):

            values = Pair(vals.first.second, vals.second)
            given_lambda = do_lambda_form(values, env)
            env.bindings[to_be_bound] = given_lambda
            return to_be_bound
        else:
            raise SchemeError()
...
def do_let_form(vals, env):
    ...
    for val in bindings:
        ...
        values = Pair(scheme_eval(val.second.first,env), values)
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    "*** YOUR CODE HERE ***"
    predicate = scheme_eval(vals.first, env)
    if scheme_true(predicate):

        return vals.second.first
    elif not vals.second.second:
        return okay
...
def do_and_form(vals, env):
    ...
    for val in vals:
        compare = scheme_eval(val, env)
        if scheme_false(compare):
            return compare
        elif val == vals[len(vals)-1] and scheme_true(compare):
            return compare
...
def do_or_form(vals, env):
    ...
    for val in vals:
        ...
        if val == vals[len(vals)-1] and scheme_true(compare):
            return compare
        elif scheme_true(compare): # compare == 0 
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            ...
            if len(clause.second) > 1:
                returned = Pair('begin',clause.second)
            elif len(clause.second) == 0:
                returned = test
            else:
                returned = clause.second.first
            return returned
    return okay
...
def do_begin_form(vals, env):
    ...
    while i< len(vals)-1:
        ...
        returned = scheme_eval(vals[i],env)
        i +=1
...
def check_formals(formals):
    ...
    for formal in formals:
        if not scheme_symbolp(formal):
            raise SchemeError('not a valid formal parameter')
        if formal in formals_list:
            raise SchemeError('cannot have two of same formal parameter')
