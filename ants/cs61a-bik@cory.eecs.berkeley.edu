...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            elif self.ant.container == False and insect.ant == None and insect.container == True:
...
class Ant(Insect):
    ...
    blocks_path = True
    container = False
    ant=None                           #check for this later. 

    def __init__(self, armor=1):
        """Create an Ant with an armor quantity."""
        Insect.__init__(self, armor)
        self.damage_doubled = 0
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        orginal_place = self.place
        while self.place!=hive:
            if random_or_none(self.place.bees) and self.distance >= self.min_range and self.distance <= self.max_range:
                bee = random_or_none(self.place.bees)
                self.place=orginal_place
                return bee
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        list_hurt_bees = self.place.bees
        if self.armor <= 0:
            for each_bee in list_hurt_bees[:]:
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        # colony_queen.add_insect(QueenAnt)
        # ant_queen.add_insect(QueenAnt)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        Insect.__init__(self, armor)
        QueenAnt.times_implemented +=1
        self.implementnumber = QueenAnt.times_implemented
    ...
    def action(self, colony):
        ...
        if self.implementnumber >1:               #die imposter!
            ...
            ThrowerAnt.action(self,colony)                      #throw shizz at the bee

            for place in colony.places:
                ant_place = colony.places[place]

                if type(ant_place.ant) == QueenAnt:
                    ant_queen = ant_place
                    current_tunnel_letters = list(place)[:9]
                    current_tunnel = ''.join(current_tunnel_letters)
                elif type(ant_place.ant) == BodyguardAnt:                          #Special BodyguardAnt case
                    if type(ant_place.ant.ant) == QueenAnt:                  #
                        ant_queen = ant_place                                #
                        current_tunnel_letters = list(place)[:9]             #
                        current_tunnel = ''.join(current_tunnel_letters)     #


            colony.queen = QueenPlace(colony.queen,ant_queen)    #replace colony.queen to Queenplace object.     

            for place in colony.places:
                ant_place = colony.places[place]
                
                if current_tunnel in place:
                    if type(ant_place.ant) == QueenAnt:                                 #keeping damage for Queen the same
                        ant_place.ant.damage = ant_place.ant.damage
                    elif ant_place.ant and ant_place.ant.damage_doubled == 0:                #doubling damage
                        ant_place.ant.damage = ant_place.ant.damage * 2
                        ant_place.ant.damage_doubled += 1
                        if type(ant_place.ant) == BodyguardAnt:                       #Special BodyguardAnt case
                            if type(ant_place.ant.ant) != QueenAnt:                           #
                                ant_place.ant.ant.damage = ant_place.ant.ant.damage *2        #
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    applying = effect(bee.action)
    orginal_action = bee.action
